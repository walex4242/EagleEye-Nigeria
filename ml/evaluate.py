...
def evaluate() -> None:
    """Run full evaluation on the validation set with detailed metrics."""
    ...
    print("\n  ── Per-Class Metrics ──")
    print(
        f"  {'Class':<30} {'Precision':>10} {'Recall':>10} "
        f"{'F1':>10} {'Support':>10}"
    )
    print(f"  {'─' * 72}")
    ...
    print("\n  ── Confidence Analysis ──")
    if correct_confs:
        correct_arr = np.array(correct_confs)
        print(
            f"  Correct predictions:  "
            f"avg={float(np.mean(correct_arr)):.3f}, "
            f"min={float(np.min(correct_arr)):.3f}, "
            f"max={float(np.max(correct_arr)):.3f}"
        )
    if wrong_confs:
        wrong_arr = np.array(wrong_confs)
        print(
            f"  Wrong predictions:    "
            f"avg={float(np.mean(wrong_arr)):.3f}, "
            f"min={float(np.min(wrong_arr)):.3f}, "
            f"max={float(np.max(wrong_arr)):.3f}"
        )
